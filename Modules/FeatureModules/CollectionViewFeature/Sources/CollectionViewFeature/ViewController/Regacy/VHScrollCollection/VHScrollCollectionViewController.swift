...
final class VHScrollCollectionViewController: UIViewController {
    ...
    @IBOutlet private var collectionView: UICollectionView! {
        didSet {
            collectionView.register(Cell.nib(bundle: .module), forCellWithReuseIdentifier: Cell.className)
...
extension VHScrollCollectionViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        (collectionView.dequeueReusableCell(withReuseIdentifier: Cell.className, for: indexPath) as! Cell).configure(sampleModel: samples[indexPath.item])
    }
}
