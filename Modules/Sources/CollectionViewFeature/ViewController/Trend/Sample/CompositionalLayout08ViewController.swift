    ...
    override func loadView() {
        ...
        declarative {
            UIStackView.vertical {
                UISegmentedControl(items: ["First", "Second"]).apply {
...
private extension CompositionalLayout08ViewController {
    var layout: UICollectionViewLayout {
        UICollectionViewCompositionalLayout.waterfall(
            contentInsetsInSection: { _ in .zero },
            headerHeightInSection: { _ in 0 },
            footerHeightInSection: { _ in 80 },
            horizontalSpaceInSection: { _ in 10 },
            verticalSpaceInSection: { _ in 10 },
            numberOfColumnInSection: {[weak self] _ in CGFloat(self!.segmentControll.selectedSegmentIndex + 1) },
            numberOfItemsInSection: {[weak self] section in
                ...
                guard
                    let self,
                    let size = self.items[itemIndex].image?.size
                else { return 0 }
                let aspect = CGFloat(size.height) / CGFloat(size.width)
                let height = cellWidth * aspect
                return height
            })
//        UICollectionViewCompositionalLayout.waterfall(
//            footerHeight: 80,
